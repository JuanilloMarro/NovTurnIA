-- ============================================================
-- 019: Eliminar overloads de las 4 RPCs de inteligencia.
-- Las migraciones 014, 015 y 016 crearon múltiples versiones
-- con distintas firmas (1, 2 y 3 parámetros). PostgREST no
-- puede resolver la ambigüedad y devuelve error en cada llamada.
-- Se eliminan todas las firmas previas y queda solo la de 016.
-- ============================================================

-- ── get_patient_ltv ──────────────────────────────────────────
DROP FUNCTION IF EXISTS get_patient_ltv(UUID);
DROP FUNCTION IF EXISTS get_patient_ltv(UUID, INT);
-- Recrear la versión canónica (3 params, igual que 016)
...
-- ── get_retention_rate ───────────────────────────────────────
DROP FUNCTION IF EXISTS get_retention_rate(UUID);
DROP FUNCTION IF EXISTS get_retention_rate(UUID, INT);
...
DROP FUNCTION IF EXISTS get_service_analytics(UUID);
DROP FUNCTION IF EXISTS get_service_analytics(UUID, INT);
...
-- ── get_appointment_prediction ───────────────────────────────
DROP FUNCTION IF EXISTS get_appointment_prediction(UUID);
DROP FUNCTION IF EXISTS get_appointment_prediction(UUID, INT);
