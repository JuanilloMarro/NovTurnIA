...
- [ ] **[TÚ] RES-2 · Aplicar la migración y desplegar `onboard-tenant`** — el código ya está escrito en el repo: migración `20260806010000_res2_provision_tenant.sql` + `onboard-tenant/index.ts` reescrito (auth primero, las 3 escrituras de Postgres en una transacción). **No puedo aplicarlo yo**: la migración necesita el CLI de Supabase o el MCP autenticado, y la función necesita `supabase functions deploy onboard-tenant`. Hasta que se despliegue, producción sigue con las 4 escrituras sueltas. Verificar después con un alta de prueba que falle a propósito.
...
- [ ] **[IA] A7 · Revisar el alcance real de los 65 nodos restantes** — cobertura hoy **73/138**. ⚠️ **A8 NO destraba esto: lo invierte.** Un nodo con `continueRegularOutput` **no falla**, así que la ejecución no muere y **el Error Trigger nunca se dispara**. Con A8 cableado, dejar un nodo *sin* `onError` es ahora la opción RUIDOSA: muere, y el dueño recibe la notificación con el cliente y lo que pedía. Agregarles `continueRegularOutput` en bloque los volvería silenciosos otra vez. Lo que queda no es "aplicar la regla a 65 nodos" sino decidir, para cada uno, entre morir con aviso (hoy) o `continueErrorOutput` con una rama explícita. Criterio en [Observaciones y Decisiones §P2](Observaciones%20y%20Decisiones.md).
