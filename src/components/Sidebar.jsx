...
export default function Sidebar() {
    ...
    return (
        <>
            {isSidebarOpen && (
                <div
                    className="md:hidden fixed inset-0 bg-navy-900/10 backdrop-blur-md z-[19] cursor-pointer"
                    onClick={toggleSidebar}
                />
            )}

            <aside className={`absolute left-0 top-0 bottom-0 w-[240px] p-6 flex flex-col z-20 bg-white/30 backdrop-blur-xl border-r border-white/60 md:bg-transparent md:backdrop-blur-none md:border-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
