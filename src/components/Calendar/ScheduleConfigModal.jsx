    ...
    return createPortal(
        <div className="fixed inset-0 bg-navy-900/10 backdrop-blur-md z-[200] flex items-center justify-center p-4" onClick={onClose}>
            <div onClick={e => e.stopPropagation()}
                className="bg-white/40 backdrop-blur-2xl border border-white/60 rounded-[32px] shadow-[0_8px_32px_rgba(26,58,107,0.15)] w-full max-w-lg overflow-hidden animate-fade-up flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-3 space-y-5 overflow-y-auto custom-scrollbar">
                    {/* Cupo diario */}
                    ...
                    <div>
                        <p className="text-[11px] font-bold text-navy-700/50 uppercase tracking-widest mb-2 px-1">Excepciones ({exceptions.length})</p>
                        {loading ? (
                            <p className="text-[11px] font-semibold text-navy-700/40 text-center py-4">Cargando…</p>
                        ) : exceptions.length === 0 ? (
                            <p className="text-[11px] font-semibold text-navy-700/40 text-center py-4">Sin excepciones. El horario normal aplica todos los días.</p>
                        ) : (
                            <div className="space-y-2">
                                {exceptions.map(e => (
                                    <div key={e.id} className="flex items-center gap-3 bg-white/40 border border-white/60 rounded-2xl px-3 py-2.5">
