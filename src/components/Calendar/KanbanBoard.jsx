...
export default function KanbanBoard({ appointments = [], onAppointmentClick, reload, viewMode = 'week', anchorDate, weekStart }) {
    ...
    return (
        <div className="h-full flex flex-col gap-3 p-4">
            ...
            <div className="flex-1 flex gap-4 overflow-x-auto custom-scrollbar p-3 min-h-0">
                {columns.map(col => {
                    const Icon = col.icon;
                    return (
                        <div
                            key={col.id}
                            className="relative flex-1 min-w-[280px] flex flex-col bg-white/40 backdrop-blur-2xl border border-white/60 rounded-[24px] shadow-md overflow-hidden transition-colors duration-300"
                        ...
                        >
                            ...
                            <div className="relative z-10 flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3">
                                {col.cards.length === 0 ? (
                                    ...
                                    col.cards.map(card => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`group relative overflow-hidden bg-white/40 backdrop-blur-2xl border border-white/60 p-4 rounded-2xl shadow-md hover:bg-white/60 transition-all duration-300 ${isTerminal ? 'cursor-default opacity-80' : 'cursor-grab active:cursor-grabbing'}`}
                                            >
