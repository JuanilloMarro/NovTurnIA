import { Bell, Moon, Sun, Monitor, LogOut, Calendar, UserPlus, Trash2, Bot, Menu, Clock, Siren, Home, MessageSquareWarning, PhoneCall, Check, Building2, ShieldAlert, Lock, ShieldCheck, RotateCcw } from 'lucide-react';
                                ...
                                {activityLog.length === 0 ? (
                                    ...
                                    activityLog.map(entry => (
                                        <NotifCard
                                            ...
                                            onMarkUnread={() => markOneUnread(entry.id)}
                                ...
                                <div className="flex gap-1">
                                    {themeOptions.map(opt => (
                                        <button
                                            ...
                                            className={`flex-1 flex flex-col items-center gap-1 py-2 px-1 rounded-xl text-[10px] font-bold transition-all ${theme === opt.id
...
function NotifCard({ entry, onMarkRead, onMarkUnread, onDelete }) {
    const isBotError = entry.type === 'error_ia' || entry.type === 'bot_error' || entry.type === 'ia_fallo' || entry.type === 'ia_error' || entry.type === 'bot_fail' || entry.type === 'ia_failed';
    const isBotWarning = entry.type === 'bot_reactivate' || entry.type === 'bot_pause' || entry.type === 'ia_pausa' || entry.type === 'bot_paused' || entry.type === 'ia_pausada' || entry.type === 'human_takeover' || entry.type === 'takeover_active';
    ...
    const isPlanLimit = entry.type === 'plan_limit';
    const isPlanBlocked = entry.type === 'limited_plan';
                            ...
                            : isPideHumano ? 'bg-sky-500/90'
                                : isAptNew ? 'bg-emerald-500/90'
                                    : isPatientNew ? 'bg-emerald-500/90'
                                        : isPlanLimit ? 'bg-amber-500/90'
                                            : isPlanBlocked ? 'bg-amber-500/90'
                                                : 'bg-navy-900/90';

    const dotIcon = isBotError ? <MessageSquareWarning size={15} className="text-white" />
                            ...
                            : isPideHumano ? <PhoneCall size={15} className="text-white" />
                                : isAptNew ? <Calendar size={15} className="text-white" />
                                    : isPatientNew ? <UserPlus size={15} className="text-white" />
                                        : isPlanLimit ? <ShieldAlert size={15} className="text-white" />
                                            : isPlanBlocked ? <Lock size={15} className="text-white" />
            ...
            <div className={`flex flex-1 items-center gap-3 min-w-0 ${entry.read ? 'opacity-40' : 'opacity-100'}`}>
                ...
                <div className="flex-1 min-w-0 pr-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                        <span className={`font-bold text-navy-900 text-sm leading-tight truncate group-hover:whitespace-normal group-hover:overflow-visible transition-all ${entry.read ? 'text-navy-900/50' : ''}`}>
                            {entry.title}
                        </span>
                    </div>
                    {entry.message && (
                        isReminder ? (
                            ...
                            <p className="text-navy-800/65 text-[11px] truncate group-hover:whitespace-normal group-hover:overflow-visible mt-0.5 font-medium transition-all">
                                {entry.message}
                            </p>
                        )
                    )}
            ...
            <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150 z-10">
                {entry.read ? (
                    <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-xl bg-sky-50 hover:bg-sky-100 text-sky-600 border border-sky-200/60 transition-colors"
                    >
